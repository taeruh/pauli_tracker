...
//! and [bitvec_simd::BitVec] (included via the corresponding features). However, note
//! that these types of bit-vector implementations might not be the most efficient for
//! your problem, e.g., while [bitvec_simd::BitVec] uses SIMD operations, it also uses
//! the crate [smallvec](https://docs.rs/smallvec/1.10.0/smallvec/) for its inner
//! storage, which can be disadvantageous, depending on the situation. There are other
